...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QTextEdit, QAction, QFileDialog, QMainWindow, QSizePolicy, \
    QScrollBar, QSpinBox, QHBoxLayout, QVBoxLayout, QPushButton
...
class FrameGrabber(QtCore.QObject):
    ...
    def request_frame(self, target_frame):
        ...
        bytesPerPixel = 3
        img = QImage(self.frame, rgba.width, rgba.height, rgba.width * bytesPerPixel, QImage.Format_RGB888)
...
class DisplayWidget(QLabel):
    ...
    def setPixmap(self, img, index):
        ...
        # overlay = QImage('icons/actions/96/appointment-new.png')
        # painter = QPainter()
        # painter.begin(img)
        # # painter.drawImage(100, 100, overlay)
        # painter.setBrush(Qt.yellow)
        # painter.drawRect(100, 50, 50, 50)
        # painter.end()
...
class VideoPlayerWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.timeline = QScrollBar(Qt.Horizontal)
        self.frame_grabber = FrameGrabber()

        self.frame_control = QSpinBox()
    ...
    def dropEvent(self, event):
        ...
        if mime.hasUrls():
            path = str(mime.urls()[0].toLocalFile())
            self.set_file(path)
...
class MainLayout(QWidget):
    ...
    def initUI(self):
        ...
        self.videoPlayer.frame_grabber.frame_ready.connect(self.processFrame)

        self.yellow_input_x = QSpinBox()
        self.yellow_input_x.setMaximum(1920)
        self.yellow_input_x.setValue(100)
        self.yellow_input_x.valueChanged.connect(self.yellowInputChanged)
        self.yellow_input_y = QSpinBox()
        self.yellow_input_y.setMaximum(1080)
        self.yellow_input_y.setValue(100)
        self.yellow_input_y.valueChanged.connect(self.yellowInputChanged)

        yellow_button_layout = QHBoxLayout()
        yellow_button_layout.addWidget(self.yellow_input_x)
        yellow_button_layout.addWidget(self.yellow_input_y)


        hbox = QHBoxLayout()
        hbox.addWidget(self.videoPlayer)
        hbox.addLayout(yellow_button_layout)

        self.setLayout(hbox)

    def processFrame(self, img, index):
        pass

    def yellowInputChanged(self, value):
        frame = self.videoPlayer.frame_grabber.get_frame(self.videoPlayer.frame_control.value())

        rgba = frame.reformat(frame.width, frame.height, "rgb24", 'itu709')
        frame = bytearray(rgba.planes[0])
        bytesPerPixel = 3
        img = QImage(frame, rgba.width, rgba.height, rgba.width * bytesPerPixel, QImage.Format_RGB888)

        painter = QPainter()
        painter.begin(img)
        painter.setBrush(Qt.blue)
        painter.drawRect(self.yellow_input_x.value(), self.yellow_input_y.value(), 50, 50)
        painter.end()

        self.videoPlayer.display.setPixmap(img, None)
...
class App(QMainWindow):
    ...
    def initMenu(self):
        ...
        openFile = QAction(QIcon(), 'Öffnen', self)
    ...
    def showDialog(self):
        ...
        if fname[0]:
            self.main_layout.videoPlayer.set_file(fname[0])


if __name__ == "__main__":
    app = QApplication(sys.argv)
    #window = VideoPlayerWidget()
    # test_file = 'MVI_0245.MOV' #sys.argv[1]
