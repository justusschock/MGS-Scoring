...
from blocks import UnetSkipConnectionBlock, UnetSkipConnectionInceptionBlock
...
class Generator(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.threshold_gen:
            tmp.data = torch.add(torch.mul(torch.gt(tmp.data, 0.5).float(), 2).float(), -1)

        return tmp


class InceptionGenerator(torch.nn.Module):
    ...
    def _build(self):
        ...
        nf_mult = min(8, 2**self.n_blocks)
        model = UnetSkipConnectionInceptionBlock(nf_mult*self.initial_filters, self.dropout_value,
                                                 nf_mult*self.initial_filters, innermost=True)

        for i in range(self.n_blocks-1, 1, -1):
            nf_mult_out = min(8, 2**(int(i-2)))
            nf_mult_in = 8 if nf_mult_out == 8 else 2*nf_mult_out
            model = UnetSkipConnectionInceptionBlock(nf_mult_out*self.initial_filters, self.dropout_value,
                                                     nf_mult_in*self.initial_filters, model)

        model = [UnetSkipConnectionInceptionBlock(self.input_nc, self.dropout_value, self.initial_filters, model, outermost=True)]
        model += [torch.nn.Conv2d(self.input_nc, self.output_nc, kernel_size=1), torch.nn.Sigmoid()]
